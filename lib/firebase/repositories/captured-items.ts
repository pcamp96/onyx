...
export const capturedItemsRepository = {
  async create(input: Omit<CapturedItem, "id" | "createdAt">) {
    const ref = getDb().collection<CapturedItem>(COLLECTION).doc();
  ...
  async listLatest(limit = 20) {
    const snapshot = await getDb().collection<CapturedItem>(COLLECTION).orderBy("createdAt", "desc").limit(limit).get();
    return snapshot.docs.map((doc) => toPlainObject(doc.data() as unknown as CapturedItem));
  },
};
